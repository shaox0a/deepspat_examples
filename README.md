...
## Background
Deep compositional spatial models couple standard spatial covariance and extreme-value constructions with an injective warping of the spatial (and, when needed, temporal) domain. The warping is built as a composition of elemental injective mappings within a deep-learning framework. In the univariate setting, we consider (i) deformations known up to weights to be estimated and (ii) layer-wise random weights; in the multivariate setting only the former is used. Estimation and inference are performed in TensorFlow (GPU-enabled) via automatic differentiation. The examples here illustrate this paradigm by fitting stationary models in the warped space—Gaussian processes, Brown–Resnick max-stable, and r-Pareto—to reproduce the Nepal case study and simulation workflows.

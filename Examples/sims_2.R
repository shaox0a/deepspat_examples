...
pred_gp <- predict(d_gp, holes_test)
predall_gp <- predict(d_gp, holes_all)
...
nn_id_pred <- FNN::get.knnx(data = locs,
                            ...
                            k = 50)$nn.index
pred_nngp <- predict(d_nngp, holes_test, nn_id_pred)

nn_id_pred <- FNN::get.knnx(data = locs,
                            query = as.matrix(holes_all[,c("x", "y")]),
                            k = 50)$nn.index
predall_nngp <- predict(d_nngp, holes_all, nn_id_pred)
